...
// Variable: AUTHOR_BY_GOOGLE_SUB_QUERY
// Query: *[_type == "author" && id == $id][0] {       _id, id, name, username, email, image, bio    }
export type AUTHOR_BY_GOOGLE_SUB_QUERYResult = {
  _id: string;
  id: number | null;
  name: string | null;
  username: string | null;
...
// Query: *[_type == "playlist" && slug.current == $slug][0]{    _id,    title,    slug,    select[]-> {        _id,        createdAt,        title,        slug,        author-> {            _id,            name,            slug,            image,            bio        },        views,        description,        category,        image,        pitch    }    }
